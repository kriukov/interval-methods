...
f0(x, k) = mod2pi([x[1] + k*sin(x[1]) + x[2], k*sin(x[1]) + x[2]])
...
function roots(k, n, x1, x2, p1, p2, precision)
	array_sol = IntervalBox[]
	a = IntervalBox(Interval(x1, x2), Interval(p1, p2))
	for p = 0:top(a, k, n)[1]/2pi
		for q = 0:top(a, k, n)[2]/2pi
			array_sol = vcat(array_sol, krawczyk2d(x -> h(x, k, n) - IntervalBox(Interval(2pi*p), Interval(2pi*q)), a, precision))
		end
	end
